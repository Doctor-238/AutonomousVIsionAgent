...
def cube_state(*, visible: bool, parent_pad_id: str | None) -> SimpleNamespace:
    return SimpleNamespace(
        visible=visible,
        state={"parent_pad_id": parent_pad_id} if parent_pad_id is not None else {},
    )
...
class DeliveredCubeIdsTest(unittest.TestCase):
    def test_ignores_hidden_pool_or_conveyor_cubes_that_were_not_delivered(self):
    ...
    def test_counts_invisible_cubes_on_destination_pads_only(self):
        ctx = FakeSceneContext(
            {
                "cube_0": cube_state(visible=False, parent_pad_id="pad_D"),
                "cube_pool_0": cube_state(visible=False, parent_pad_id="pad_E"),
                "cube_1": cube_state(visible=False, parent_pad_id="pad_A"),
                "cube_2": cube_state(visible=True, parent_pad_id="pad_C"),
            }
        )
