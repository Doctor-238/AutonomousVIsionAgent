...
def _navigation_arrived(
    *,
    ...
    if target_kind == "cube":
        # 큐브가 이미 충분히 크면 즉시 도달 (도달 기준 면적 이상)
        if area >= arrival_area:
            return True
        # 이동 이력이 있으면서 접근 기준 면적 이상이면 도달
    ...
    if memory.held_color and memory.priority_colors and memory.held_color not in memory.priority_colors:
        ...
        if memory.pad_ready:
            return AgentDecision(
                ...
                target_color=dump_color,
                reason="Fast local policy: dumping unwanted cube.",
            )
        return AgentDecision(
            next_action="navigate_to_pad",
            target_color=dump_color,
            reason="Fast local policy: seeking pad to dump unwanted cube.",
        )
...
def _should_peek_down(target_kind: str, *, area: int, angle_deg: float, step: int) -> bool:
    ...
    return step >= 2 and area >= PAD_PEEK_AREA
...
def update_memory(
    ...
    elif is_success and action != "recover" and decision.target_color in memory.failed_attempts:
        memory.failed_attempts[decision.target_color] = 0

    if action == "recover" and decision.reason and "repeated failures" in decision.reason:
        if decision.target_color in memory.failed_attempts:
            memory.failed_attempts[decision.target_color] = 0
...
async def visual_navigate_to_target(
    ...
    *,
    ...
    print(f"Starting visual navigation to {target_color} {target_kind}")
    
    max_steps = 10 if target_kind == "pad" else 20
    for step in range(1, max_steps + 1):
        # Ensure head is at base pitch at the start of each step (in case we peeked down previously)
        await set_head_cached(ctx, memory, yaw=0.0, pitch=base_pitch)
        ...
        if not target_det:
            ...
            if target_kind == "pad":
                ...
                if known:
                    bearing = float(known.get("bearing_deg", 0.0))
                    sweep_wz = -0.6 if bearing > 0 else 0.6
                    sweep_dur = 0.8
                else:
                    idx = (step - 1) % 4
                    if idx == 0:
                        sweep_wz, sweep_dur = 0.8, 1.0
                    elif idx == 1:
                        sweep_wz, sweep_dur = -0.8, 2.0
                    elif idx == 2:
                        sweep_wz, sweep_dur = 0.8, 1.0
                    else:
                        sweep_wz, sweep_dur = 1.5, 2.1
                print(f"Nav step {step}: sweeping (wz={sweep_wz}, dur={sweep_dur})")
                move_result = await move_velocity(ctx, wz=sweep_wz, duration_s=sweep_dur)
            else:
        ...
        if _navigation_arrived(
            ...
            if target_kind == "pad":
                use_vlm = os.environ.get("MENLO_USE_VLM_HINTS", "0") == "1"
                api_key = os.environ.get("TOKAMAK_API_KEY", "")
                
                if use_vlm and api_key:
                    print("Verifying if wooden pad is actually present under the sign using VLM...")
                    prompt = (
                    ...
                    if "trouble reaching" in reply_lower or "fallback" in reply_lower:
                        print("VLM unavailable. Assuming pad is valid.")
                    elif "no" in reply_lower.split() or reply_lower.strip().startswith("no"):
                    ...
                    print("Verifying if wooden pad is actually present under the sign locally...")
                    jpeg = await get_camera_frame(ctx)
                    
                    def _check_wood(jpeg_bytes: bytes) -> bool:
                        try:
                            import cv2
                            import numpy as np
                            ...
                            h, w = mask.shape
                            roi = mask[int(h*0.4):, int(w*0.2):int(w*0.8)]
                            brown_pixels = cv2.countNonZero(roi)
                            print(f"Local wooden pad check: found {brown_pixels} brown pixels.")
                            return brown_pixels > 1000
                        except Exception as e:
                            print(f"Local pad check error: {e}")
                            return True
                    
                    if not _check_wood(jpeg):
                        print("No wooden pad found locally! Marking this pad as fake/invalid.")
                        memory.failed_attempts[target_color] = memory.failed_attempts.get(target_color, 0) + 1
                        _clear_nav_track(memory)
                        await set_head_cached(ctx, memory, yaw=0.0, pitch=base_pitch)
                        return False
